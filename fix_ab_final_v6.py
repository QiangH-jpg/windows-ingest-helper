...
"""
A/B 完整版样片最终修复 v6
策略：使用 filter_complex 替代 concat，避免重复帧问题
"""
...
def assemble_v6(clip_order, audio_path, srt_path, output_path, version_name):
    print(f"\n【{version_name}视频组装】")
    ...
    filter_complex = ''.join(filter_parts) + f'concat=n={len(clip_order)}:v=1:a=0[outv]'
    ...
    cmd = [FFMPEG, '-y'] + inputs + [
        ...
        '-vf', f"[outv]subtitles='{srt_escaped}':force_style='Alignment=2,MarginV=20,MarginL=40,MarginR=40,FontName=WenQuanYi Micro Hei,FontSize=26,PrimaryColour=&HFFFFFF,OutlineColour=&H202020,BorderStyle=1,Outline=1'",
        '-map', '[outv]', '-map', f'{len(clip_order)+1}:a',
        '-t', str(audio_dur),
    ...
    print(f"  执行 ffmpeg (filter_complex, {len(clip_order)}个镜头)...")
...
def main():
    print("=" * 60)
    print("A/B 完整版样片最终修复 v6（filter_complex 替代 concat）")
    print("=" * 60)
    
    outdir = os.path.join(PROJECT_ROOT, 'output_ab_final_v6')
    ...
    out_a = os.path.join(outdir, 'A_rule_final_v6.mp4')
    assemble_v6(a_order, tts_path, srt_path, out_a, "A 版规则基线")
    
    # B 版：包含 clip_11
    b_order = [5, 6, 7, 8, 10, 9, 0, 1, 2, 3, 4, 11, 12]
    out_b = os.path.join(outdir, 'B_ai_final_v6.mp4')
    assemble_v6(b_order, tts_path, srt_path, out_b, "B 版 AI 驱动")
    
    # 复制到 static
    static = os.path.join(PROJECT_ROOT, 'static')
    import shutil
    shutil.copy(out_a, os.path.join(static, 'A_rule_final_v6.mp4'))
    shutil.copy(out_b, os.path.join(static, 'B_ai_final_v6.mp4'))
    
    print("\n" + "=" * 60)
    print("✅ 完成")
    print("=" * 60)
    print(f"A 版：http://47.93.194.154:8088/static/A_rule_final_v6.mp4")
    print(f"B 版：http://47.93.194.154:8088/static/B_ai_final_v6.mp4")
