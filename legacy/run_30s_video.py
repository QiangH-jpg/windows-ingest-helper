...
def run_30s_video():
    ...
    for i, path in enumerate(materials):
        ...
        for c in clips:
            c['source_index'] = i
        all_clips.extend(clips[:3])  # 每个素材最多取3段
        print(f"  素材{i} 切得 {len(clips[:3])} 个clip")
    
    # 交错选片：每个素材轮流取
    clips_by_source = {0: [], 1: [], 2: []}
    for c in all_clips:
    ...
    for round_idx in range(3):
        for src in sorted(clips_by_source.keys()):
            if len(clips_by_source[src]) > round_idx:
                selected.append(clips_by_source[src][round_idx])
    
    selected = selected[:8]  # 8段 x 5秒 = 40秒
    sources = list(set(c['source_index'] for c in selected))
    print(f"交错选片: {len(selected)} clips, sources: {sources}")
    
    # 预估时长
    est_duration = len(selected) * 5
    print(f"预估时长: {est_duration}秒")
    ...
    # TTS - 40秒左右的稿件
    script = """新闻短视频自动成片系统验证测试。
本视频使用三段真实素材，通过智能选片算法自动拼接而成。
第一段素材展示城市生活场景，画面生动自然。
第二段素材呈现自然风光，色彩丰富层次分明。
第三段素材记录人文活动，场景切换流畅。
系统已完成转码、切段、配音、字幕合成等全流程自动化处理。
本次验证标志着自动成片技术从实验走向产品化应用。
感谢观看本测试视频。"""
    ...
    processor.assemble_video(selected, tts_path, srt_path, output_path, target_duration=60, keep_concat=True)
