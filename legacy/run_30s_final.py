...
total_clip_duration = len(selected) * 5
print(f"交错选片: {len(selected)} clips, 预计视频时长: {total_clip_duration}秒")
...
with open(timeline_path, 'w') as f:
    json.dump({
        ...
        'clip_duration_total': total_clip_duration,
...
本次验证采用交错选片算法，确保多个素材轮流入选最终成片。
每个素材都贡献了独特的画面内容，保证了视频的视觉多样性。
...
print(f"TTS时长: {tts_meta['total_duration']}秒")
print(f"预计最终时长: ~{tts_meta['total_duration']:.0f}秒")
print(f"\n需要执行ffmpeg合成视频")
print(f"task_id: {task_id}")