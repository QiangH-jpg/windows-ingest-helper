...
"""
A/B 完整版样片最终修复 v7
策略：concat 但每个 clip 先重新编码，避免重复帧
"""
...
def assemble_v7(clip_order, audio_path, srt_path, output_path, version_name):
    print(f"\n【{version_name}视频组装】")
    audio_dur = get_duration(audio_path)
    print(f"  音频时长：{audio_dur:.2f}秒")
    
    total_dur = sum(ORIGINAL[i] for i in clip_order)
    print(f"  画面总时长：{total_dur:.2f}秒")
    
    # 先重新编码每个 clip，然后 concat
    ...
    with open(concat, 'w') as f:
        for i, idx in enumerate(clip_order):
            ...
            # 强制重新编码
            cmd = [FFMPEG, '-y', '-i', src, '-c:v', 'libx264', '-preset', 'medium', '-crf', '23',
                   '-r', '30', '-keyint_min', '1', '-g', '30', '-an', dst]
    ...
    print(f"  执行 ffmpeg (重新编码{len(clip_order)}个镜头)...")
...
def main():
    print("=" * 60)
    print("A/B 完整版样片最终修复 v7（重新编码每个 clip）")
    print("=" * 60)
    
    outdir = os.path.join(PROJECT_ROOT, 'output_ab_final_v7')
    ...
    out_a = os.path.join(outdir, 'A_rule_final_v7.mp4')
    assemble_v7(a_order, tts_path, srt_path, out_a, "A 版规则基线")
    
    # B 版
    b_order = [5, 6, 7, 8, 10, 9, 0, 1, 2, 3, 4, 11, 12]
    out_b = os.path.join(outdir, 'B_ai_final_v7.mp4')
    assemble_v7(b_order, tts_path, srt_path, out_b, "B 版 AI 驱动")
    
    # 复制到 static
    static = os.path.join(PROJECT_ROOT, 'static')
    import shutil
    shutil.copy(out_a, os.path.join(static, 'A_rule_final_v7.mp4'))
    shutil.copy(out_b, os.path.join(static, 'B_ai_final_v7.mp4'))
    
    print("\n" + "=" * 60)
    print("✅ 完成")
    print("=" * 60)
    print(f"A 版：http://47.93.194.154:8088/static/A_rule_final_v7.mp4")
    print(f"B 版：http://47.93.194.154:8088/static/B_ai_final_v7.mp4")
